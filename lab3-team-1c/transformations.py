...
def generate_message_broker(name):
    path = f'skeleton/{name}'
    os.makedirs(path, exist_ok=True)
    with open(os.path.join(path, 'app.py'), 'w') as f:
        f.write(textwrap.dedent(f"""\
...
def generate_radio_gateway(name):
    path = f'skeleton/{name}'
    os.makedirs(path, exist_ok=True)
    with open(os.path.join(path, 'app.py'), 'w') as f:
        f.write(textwrap.dedent(f"""\
            ...
            @app.route('/health')
            def health():
                return jsonify(status='ok', service='{name}', protocol='GSM-R/Euroradio')
...
def generate_docker_compose(components):
    ...
    db_names = [n for n, (t, ct) in sorted_items if ct in ('database', 'event_store')]
    ...
    with open(os.path.join(path, 'docker-compose.yml'), 'w') as f:
        ...
        for i, (name, (tier, ctype)) in enumerate(sorted_items):
            ...
            if ctype in ('database', 'event_store'):
                ...
                f.write("    ports:\n")
                # ADR-002: Dynamic port to prevent BindError
                ...
                # Flask services: api_gateway, load_balancer, message_broker,
                # radio_gateway, microservice, authority_service
        ...
        # ADR-003: Flat bridge network for DNS resolution
        f.write("\nnetworks:\n  default:\n    driver: bridge\n")
...
GENERATORS = {
    ...
    'load_balancer': generate_load_balancer,
    'message_broker': generate_message_broker,
    'radio_gateway': generate_radio_gateway,
...
def _collect_elements(elements, components, db_map):
    """Recursively collect components from a list of elements."""
    db_names_local = []
    for e in elements:
        if e.__class__.__name__ == 'Component':
            components[e.name] = (e.tier, e.type)
            if e.type in ('database', 'event_store'):
                db_names_local.append(e.name)

    # Build DB mapping for microservices using heuristic (ADR-001)
    for name, (tier, ctype) in components.items():
        if ctype == 'microservice' and name not in db_map:
            for db in db_names_local:
                if db.replace('_db', '') in name:
                    db_map[name] = db
                    break
...
# Traverses the model and prints a structured
# report of all quality annotations declared
# on components and connectors.
# ─────────────────────────────────────────────
def print_quality_summary(model):
    print("\n=== Quality Annotation Summary ===\n")
    print("Components:")
    
    # helper generator to yield all elements even from SoS blocks
    def gather_elements(m):
        for e in getattr(m, 'elements', []):
            yield e
        for sys in getattr(m, 'systems', []):
            for e in getattr(sys, 'elements', []):
                yield e

    for e in gather_elements(model):
        if e.__class__.__name__ != 'Component':
            continue
        p = e.properties
        if p is None:
            print(f"  [{e.tier:14}] {e.name:25} (no annotations)")
            continue
        parts = []
        if p.availability: parts.append(f"availability={p.availability}")
        if p.performance: parts.append(f"performance={p.performance}")
        if p.security: parts.append(f"security={p.security}")
        if p.replicas: parts.append(f"replicas={p.replicas}")
        if p.mem_limit: parts.append(f"mem_limit={p.mem_limit}")
        if p.cpu_limit: parts.append(f"cpu_limit={p.cpu_limit}")
        print(f"  [{e.tier:14}] {e.name:25} {', '.join(parts)}")
    ...
    for e in gather_elements(model):
        if e.__class__.__name__ != 'Connector':
            continue
        p = e.properties
        src_component = getattr(e, 'from', None)
        if src_component is None:
            # textx sometimes appends underscore if it's a python keyword
            src_component = getattr(e, 'from_')
            
        label = f"{src_component.name} -> {e.to.name}"
        if p is None:
            print(f"  [{e.type:20}] {label}")
            continue
        parts = []
        if p.timeout_ms: parts.append(f"timeout_ms={p.timeout_ms}")
        if p.encrypted: parts.append(f"encrypted={p.encrypted}")
        if p.rate_limit: parts.append(f"rate_limit={p.rate_limit}")
        annotation = f" ({', '.join(parts)})" if parts else ""
        print(f"  [{e.type:20}] {label}{annotation}")
    print("\n==================================\n")
...
def apply_transformations(model):
    ...
    _collect_elements(model.elements, components, db_map)

    # Collect from SoS subsystems
    for system in model.systems:
        _collect_elements(system.elements, components, db_map)

    # Rebuild full DB list for final mapping pass
    all_db_names = [n for n, (t, ct) in components.items() if ct in ('database', 'event_store')]
    for name, (tier, ctype) in components.items():
        if ctype == 'microservice' and name not in db_map:
    ...
    # Generate artifacts for each component
    for name, (tier, ctype) in components.items():
